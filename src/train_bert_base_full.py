...
class CustomBERTBaseClassifier(nn.Module):
    def __init__(self, n_meta=0):
        ...
        self.linear_out = nn.Linear(768 + n_meta, 1)
        self.linear_aux_out = nn.Linear(768 + n_meta, 6)
...
def train(x_train, x_train_meta, y_train, y_identity_train, all_ids, params, loss_func):
    ...
    if USE_META_FEATURES:
        model = CustomBERTBaseClassifier(n_meta=x_train_meta.shape[1])
    else:
        model = CustomBERTBaseClassifier(n_meta=0)
    model.zero_grad()
    model.cuda()
    ...
    for epoch in mb:
        ...
        torch.save(model.state_dict(), f'./model/bert_base_full_epoch{epoch+1}_seed{SEED}_v{VERSION}.bin')
...
def main():
    ...
    if PREPROCESSING:
        ...
        with timer('Tokenization'):
            ...
            joblib.dump(sequences, './cache/bert_base_sequences.pkl')
        ...
        with timer('Load preprocessed data'):
            ...
            sequences = joblib.load('./cache/bert_base_sequences.pkl')
