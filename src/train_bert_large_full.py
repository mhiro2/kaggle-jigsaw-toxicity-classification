...
class CustomBERTLargeClassifier(nn.Module):
    def __init__(self, n_meta=0):
        super().__init__()
        bert_model = BertForSequenceClassification.from_pretrained('bert-large-uncased', num_labels=1)

        self.bert = bert_model.bert
        self.dropout = bert_model.dropout  # Dropout(0.1)

        self.n_meta = n_meta
        if n_meta > 0:
            self.linear_meta = nn.Linear(n_meta, n_meta)

        self.linear_out = nn.Linear(1024 + n_meta, 1)
        self.linear_aux_out = nn.Linear(1024 + n_meta, 6)

    def forward(self, input_ids, x_meta, token_type_ids=None, attention_mask=None, labels=None):
        _, pooled_output = self.bert(input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False)
        pooled_output = self.dropout(pooled_output)

        if self.n_meta > 0:
            meta_output = self.linear_meta(x_meta)
            conc = torch.cat([pooled_output, meta_output], 1)
        else:
            conc = pooled_output
...
def train(x_train, x_train_meta, y_train, y_identity_train, all_ids, params, loss_func):
    ...
    if USE_META_FEATURES:
        model = CustomBERTLargeClassifier(n_meta=x_train_meta.shape[1])
    else:
        model = CustomBERTLargeClassifier(n_meta=0)
    model.zero_grad()
    model.cuda()
    ...
    for epoch in mb:
        ...
        torch.save(model.state_dict(), f'./model/bert_large_full_epoch{epoch+1}_seed{SEED}_v{VERSION}.bin')
...
def main():
    ...
    if PREPROCESSING:
        ...
        with timer('Tokenization'):
            ...
            joblib.dump(sequences, './cache/bert_large_sequences.pkl')
        ...
        with timer('Load preprocessed data'):
            ...
            sequences = joblib.load('./cache/bert_large_sequences.pkl')
